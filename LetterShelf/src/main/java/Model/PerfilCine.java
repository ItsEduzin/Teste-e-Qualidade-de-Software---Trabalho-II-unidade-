...
public class PerfilCine {
    ...
    public PerfilCine(int duracaoMin, int duracaoMax, ClassIndicativa classIndMax, Map<Genero, Double> pesos,
            Set<Idioma> idiomasConhecidos, Map<Filme, Integer> notas, Set<Filme> assistidos)
            throws DuracaoInvalidaException, PesoInvalidoException {

        if (duracaoMin < 0 || duracaoMax <= duracaoMin) {
            throw new DuracaoInvalidaException("Duração mínima ou máxima inválida");
        }

        for (Double peso : pesos.values()) {
            if (peso < 0 || peso > 1) {
                throw new PesoInvalidoException("Os pesos devem estar entre 0 e 1");
            }
        }
        ...
        for (Genero genero : Genero.values()) {
            if (pesos.getOrDefault(genero, 0.0) != 0) {
                GenBit += (1 << bit);
            }
            bit++;
        }
